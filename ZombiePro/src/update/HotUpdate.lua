...
function HotUpdate:create(callback, parent)   
    local ret = HotUpdate.new()
    ret:init(callback, parent)
    return ret
end

function HotUpdate:ctor()
    local url = "http://dj-dpu.oss-cn-beijing.aliyuncs.com"
    self.serverVersionListUrl = url .. "/myTest/uploadVersionList.json"
    self.hotUpdateServerDir = url .. "/myTest/"
...
function HotUpdate:init(callback, parent)
...
function HotUpdate:saveLocalVersion(versionInfo) 
    ...
    io.writefile(self.myWritePath .. "versioninfo.json", info)
end

--读取本地版本信息
function HotUpdate:readVersion()
    if not cc.FileUtils:getInstance():isFileExist(self.myWritePath .. "versioninfo.json") then 
        print('热更新目录的配置文件不存在')
        local fullPath = cc.FileUtils:getInstance():fullPathForFilename("res/versioninfo.json");
        ...
        print('热更新目录的配置文件已存在')
        local info = cc.FileUtils:getInstance():getStringFromFile(self.myWritePath .. "versioninfo.json");    
        info = json.decode(info);
