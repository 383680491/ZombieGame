...
--增加遮罩
function BaseView:addLayerMask(opacity) 
    opacity = opacity or 0
    --全屏的黑色透明的层
    if not self.layerMask then 
        -- self.layerMask = display.newLayer({r = 0, g = 0, b = 0, a = opacity})
        self.layerMask = display.newColorLayer({r = 0, g = 0, b = 0, a = opacity})
...
function BaseView:setMoudleId(id) 
	self.moduleId = id
end
