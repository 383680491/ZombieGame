...
bool MirTextNodeFormatter::multilineText(MirTextNode *theLabel)
{
    ...
    MirTextNode::LetterInfo* info = nullptr;
...
bool MirTextNodeFormatter::alignText(MirTextNode *theLabel)
{
    int i = 0;
...
bool MirTextNodeFormatter::createStringSprites(MirTextNode *theLabel)
{
    // check for string
    unsigned int stringLen = theLabel->_currentUTF16String.size();
    ...
    if (nullptr == theLabel->_horizontalKernings)
    {
        return false;
    }
    ...
    auto strWhole = theLabel->_currentUTF16String;
    auto fontAtlas = theLabel->_batchDrawLabel->getFontAtlas();
