...
GameMap = class("GameMap", function(path)
    return cc.TMXTiledMap:create(path)
end)

GameMap.__index = GameMap
GameMap.__instance = nil

function GameMap:create(parent, path)
    local ret = GameMap.new(path)
    ret:init(parent)
    ret:registTouch()
...
--在天梯上
function GameMap:isHeroInGadget(heroPos)
    if not self._battle.hero:getGadget() then
        for _, gadget in ipairs(self.gadgetList) do
            if gadget:getStatus() == GADGET_ACTIVE then
                local gadgetRect = gadget:getCollisionRect();
                if cc.rectContainsPoint(gadgetRect, heroPos) then
                    local gadgetLevel = gadget:getPositionY() + gadgetRect.height
                    return true, gadgetLevel, gadget
                end
            end
        end
    end
...
function GameMap:isBombInGadget(bombPos)
    for _, gadget in ipairs(self.gadgetList) do
        if gadget:getStatus() == GADGET_ACTIVE then
            local gadgetRect = gadget:getCollisionRect();
            if cc.rectContainsPoint(gadgetRect, bombPos) then
                local gadgetLevel = gadget:getPositionY() + gadgetRect.height;
                return true, gadgetLevel, gadget
            end
        end
...
--弹簧
function GameMap:isHeroInSpring(heroPos, direction)
    if not self._battle.hero:getSpring() then
        for _, spring in ipairs(self.SpringList) do
            if spring:getStatus() == SPRING_ACTIVE then
                local springRect = spring:getCollisionRect();
                if cc.rectContainsPoint(springRect, heroPos) then
                    local springLevel = 0;
                    if direction == 'r' then
                        springLevel = springRect.x
                    elseif direction == 'l' then
                        springLevel = springRect.x + springRect.width
                    else
                        springLevel = spring:getPositionY() + spring:getSize().height;
                    end

                    return true, springLevel, spring
                end
            end
        end
    end
...
--cclog('tileCoord (%d, %d)', tileCoord.x, tileCoord.y)
    ...
    --[[local gid = self.blockLayer:getTileGIDAt(tileCoord);
    if gid > 0 then
    ...
    --[[local gid = self.coinLayer:getTileGIDAt(tileCoord);
    if gid > 0 then
    ...
    --[[local gid = self.coinLayer:getTileGIDAt(tileCoord);
    if gid > 0 then
        return TILE_TYPE_BOX;
    end

    local gid = self.coinLayer:getTileGIDAt(tileCoord);
    if gid > 0 then
        return TILE_TYPE_PROP;
    end]]
