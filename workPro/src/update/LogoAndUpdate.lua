...
function LogoAndUpdate:initUI() 
    ...
    local str = "<image src='face/002.png' /> <font name='text_normal.ttf' size='24' link=true linkparam=2 color=#00ff00>你好啊{value}</font> <newline/> <font name='text_normal.ttf' size='24'>我的名字{昵称}，我有{金钱}金币</font>"
    local msg = {
        replaceValueList = {
            ' 哈哈',
            ' 嘻嘻'
        ...
        callbackList = {
            function(arg1) 
                look(arg1, 'callbackList  value== ')
            end
        }
    }

    local TestTip = require 'utils.RichLabel'
    TestTip = TestTip:new(str, msg) -- <font name='text_normal.ttf' color='#ffffff'>hello,world</font>
    TestTip:setPosition(cc.p(100, 100))
    self:addChild(TestTip, 100, 100)
    ...
    local centerPoint = cc.p(winSize.width / 2, winSize.height / 2)
    drawNode:drawPoint(centerPoint, 10, cc.c4f(1, 0, 0, 1))

    local layerMask = display.newLayer({r = 0, g = 0, b = 0, a = 0})
    self:addChild(layerMask)

    layerMask:setTouchEnabled(true)
    local listenner = cc.EventListenerTouchOneByOne:create()  
    listenner:setSwallowTouches(true)  
    listenner:registerScriptHandler(function(touch, event)  
        local location = touch:getLocation() 
        location = cc.p(math.floor(location.x), math.floor(location.y)) 
        drawNode:drawPoint(cc.p(location.x, location.y), 10, cc.c4f(1, 1, 0, 1))

        local targetPoint
        if location.x ~= centerPoint.x then 
            local angle = math.atan((location.y - centerPoint.y) / (location.x - centerPoint.x))
            local vw = 50 * math.cos(angle)
            local vh = 50 * math.sin(angle)

            look(angle, 'angle ===')

            
            if location.x > centerPoint.x then 
                targetPoint = cc.p(location.x + vw, location.y + vh)
            else
                targetPoint = cc.p(location.x - vw, location.y - vh)
            end


            --然后垂直该线段 来回走动 假设走动50距离  通过画图可以看得出来
            local vw1 = 50 * math.sin(angle)
            local vh1 = 50 * math.cos(angle)

            drawNode:drawPoint(cc.p(centerPoint.x + vw1, centerPoint.y - vh1), 10, cc.c4f(0, 1, 0, 1))
            drawNode:drawPoint(cc.p(centerPoint.x - vw1, centerPoint.y + vh1), 10, cc.c4f(0, 1, 0, 1))
        else
            if location.y >= centerPoint.y then 
                targetPoint = cc.p(centerPoint.x, location.y + 50)
            else
                targetPoint = cc.p(centerPoint.x, location.y - 50)
            end
        end
...
look(targetPoint, 'targetPoint=')

        drawNode:drawPoint(cc.p(targetPoint.x, targetPoint.y), 10, cc.c4f(0, 1, 0, 1))

        return true  
    end, cc.Handler.EVENT_TOUCH_BEGAN )  
    local eventDispatcher = layerMask:getEventDispatcher()  
...
function LogoAndUpdate:checkHotUpdate()
    ...
    self.hotInstance = HotUpdate:create(func, self)
end
